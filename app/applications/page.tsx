...
const GRID_COLS = 'minmax(0, 2fr) minmax(0, 1fr) 110px 110px 80px 52px 32px';
...
function ApplicationCard({
  ...
  return (
    <div className={`bg-white rounded-xl ring-1 ring-zinc-200 shadow-sm overflow-hidden border-l-4 ${cfg.border}`}>
      ...
      <div
        className="w-full text-left hover:bg-zinc-50/50 transition-colors hidden lg:grid cursor-pointer"
      ...
      >
        <div className="flex items-center gap-3 px-4 py-3 min-w-0 overflow-hidden">
          ...
          <Tooltip>
            <TooltipTrigger className="truncate font-semibold text-zinc-900 text-sm">
              {grant.name}
            </TooltipTrigger>
        ...
        <div className="flex items-center px-2 py-3 min-w-0 overflow-hidden">
          <Tooltip>
            <TooltipTrigger className="flex items-center gap-1.5 text-sm text-zinc-600 truncate">
        ...
        <div className="flex items-center px-2 py-3">
          <span className="text-sm font-medium text-zinc-700 tabular-nums">{amount || '—'}</span>
        </div>
        <div className="flex items-center px-2 py-3">
          <span className="text-sm text-zinc-600 tabular-nums">{deadline || 'Open'}</span>
        </div>
        <div className="flex items-center px-2 py-3">
          <span className="text-xs text-zinc-400 tabular-nums">{formatDateShort(app.startedAt)}</span>
        </div>
      ...
      <button
        className="w-full text-left hover:bg-zinc-50/50 transition-colors lg:hidden px-4 py-4"
        onClick={() => setExpanded(e => !e)}
      >
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <h3 className="font-semibold text-zinc-900 text-sm leading-snug mb-1">{grant.name}</h3>
            <p className="flex items-center gap-1 text-xs text-zinc-500 mb-2">
            ...
            <div>
              ...
              <Section title="Key Dates" icon={CalendarDays}>
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-white rounded-lg border border-zinc-200 px-3 py-2.5">
                    <p className="text-[10px] text-zinc-400 font-medium uppercase mb-0.5">Started</p>
                    <p className="text-sm font-semibold text-zinc-800 tabular-nums">{formatDate(app.startedAt) || '—'}</p>
                  </div>
                  <div className="bg-white rounded-lg border border-zinc-200 px-3 py-2.5">
                    <p className="text-[10px] text-zinc-400 font-medium uppercase mb-0.5">Submitted</p>
                    <p className="text-sm font-semibold text-zinc-800 tabular-nums">{formatDate(app.submittedAt) || '—'}</p>
                  </div>
                  <div className="bg-white rounded-lg border border-zinc-200 px-3 py-2.5">
                    <p className="text-[10px] text-zinc-400 font-medium uppercase mb-0.5">Decided</p>
                    <p className="text-sm font-semibold text-zinc-800 tabular-nums">{formatDate(app.decidedAt) || '—'}</p>
                  </div>
                </div>
              </Section>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              {(deep.applicationOpenDate || deadline) ? (
                                <div className="text-sm font-semibold text-zinc-800">
                                  {deep.applicationOpenDate && <span>Opens {formatDate(deep.applicationOpenDate)}</span>}
                                  {deep.applicationOpenDate && deadline && <span className="text-zinc-300 mx-1">|</span>}
            ...
            {STATUS_ORDER.map(status => {
              ...
              return (
                <div key={status} id={`status-${status}`}>
                  ...
                  <div
                    className="hidden lg:grid items-center py-2 text-[10px] uppercase tracking-wider text-zinc-400 font-medium sticky top-0 bg-[#f7f5f0] z-10 border-b border-zinc-200 rounded-t-lg"
                    style={{ gridTemplateColumns: GRID_COLS }}
                  >
